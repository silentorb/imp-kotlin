...
import silentorb.imp.parsing.parser.TokenGraph
import silentorb.imp.parsing.parser.TokenizedDefinition
import silentorb.imp.parsing.parser.TokenizedImport
import silentorb.imp.parsing.parser.TokenizedParameter
...
fun toTokenGraphOld(file: Path, tokens: Tokens): ParsingResponse<TokenGraph> {
...
//  val (tokenGraph, tokenGraphErrors) = toTokenGraphOld(file, tokens, importRanges, definitionRanges)
//  return ParsingResponse(
//      tokenGraph,
...
fun toTokenGraph(file: Path, tokens: Tokens): ParsingResponse<TokenGraph> {
  val (realm, errors) = parseSyntax(file.toString(), tokens)
  val burgs = realm.burgs
  val root = burgs.values.first { it.type == BurgType.fileRoot }
  val lookUpBurg = { id: BurgId -> burgs[id]!! }
  val graph = TokenGraph(
      imports = root.children
          .map(lookUpBurg)
